...
db_dir=/share/databases/CREST/silvamod
...
# Run SWARM
swarm -f -z -t 12 -w SWARM_OTUs.fasta merged_sorted.fasta -o SWARM.swarms -s SWARM.stats -u SWARM.uc
...
blastn -db SWARM_OTUs_clean.fasta -outfmt '6 qseqid sseqid pident' -out LULU_match_list.txt -qcov_hsp_perc 95 -perc_identity 95 -query SWARM_OTUs_clean.fasta -num_threads 12

Rscript $scriptdir/LULU.R
...
megablast_silvamod138pr2.sh SWARM_OTUs_curated.fasta

classify -u -o CREST_LULU -d silvamod138pr2 -i SWARM_OTUs_curated.fasta -t SWARM_table_curated.tsv SWARM_OTUs_curated_silvamod128.xml.gz
